...
### 랜딩 보강 섹션 (`/` 로그아웃, 2026-07-12 신설 — A5 사용흐름 + A4 프라이버시)

사진/스크린샷 소스 없음 → 에디토리얼과 동일하게 **텍스트-퍼스트**. 기존 랜딩 흐름(`LandingHero → LandingFeatures → ExplorePreview`, `max-w-6xl` 캔버스에 `gap-16 md:gap-24`로 스택) 안에 두 섹션을 끼운다.

**배경 리듬(기존 관례 확인):** 현재 랜딩은 **풀블리드 bg 밴드 교차가 없다** — 전부 `bg-background` 캔버스 위에 얹히고, 섹션 구분은 **채운 패널**(Hero=그라데이션 패널)과 **열린 카드 그리드**(Features)의 교차로 만든다. 신규 섹션도 이 관례를 따른다 — **풀블리드 bg-surface 밴드 도입 금지.**

**배치 순서:** `Hero → Features → **사용 흐름(A5)** → ExplorePreview → **프라이버시(A4)`**.
- 사용 흐름은 "무엇을(Features) → 어떻게(HowTo)" 전환으로 Features 바로 뒤.
- 프라이버시는 신뢰/이의 해소라 **맨 아래**(떠나기 전 "안심하고 시작" 마무리 + 닫는 인라인 CTA). 채운 패널이라 위 그리드들과 리듬 대비.

**① 사용 흐름 `LandingHowTo` (organism 신설, 컴포넌트 재사용):**
- **`GuideStep` molecule 재사용**(신설 최소화) — 랜딩은 **4스텝 축약본**(/guide 5스텝과 별개 짧은 카피). GuideStep이 "N. " 접두를 파싱해 번호 뱃지를 만드므로 카피는 동일 포맷.
- **그리드: `<ol className="grid grid-cols-1 gap-4 md:gap-6 lg:grid-cols-2">`** — 모바일·태블릿(sm~lg) **1열 세로 스택**, `lg`+에서만 **2×2**(4스텝을 고아 행 없이 균등 분할). ⑥ 동기화: 확정치는 `lg:grid-cols-2`(2×2)이며 초안의 `lg:grid-cols-3`은 폐기.
- **태블릿 리듬 결정(⑤, 2026-07-12) — 안 (a) 채택: HowTo는 태블릿에서 1열 유지.** 근거: (1) HowTo는 **순서형(1→2→3→4)** 콘텐츠라 세로 단일 열이 읽기 순서를 강화한다(2×2는 가로→세로 순서가 모호). Features는 순서 없는 **동렬 집합**(기록/지도/경로/함께)이라 그리드가 자연스러워, 레이아웃이 "순서 vs 집합"의 의미 차이를 그대로 드러낸다. (2) 태블릿(sm~lg)에서 **Features=2열 그리드 / HowTo=1열 스택**으로 컬럼 수가 갈라져 인접 그리드 2연속의 리듬이 확실히 구분된다(검수 지적 해소). (3) borderless 변형(안 b)은 GuideStep이 **/guide와 공유되는 molecule**이라 variant prop 추가 시 /guide까지 영향 — 재사용·무(無)신설 원칙에 어긋난다. 반면 태블릿 1열 스택은 오히려 /guide의 세로 `GuideStep` 리스트(`flex flex-col`)와 구조가 일치해 "가이드 축약본" 인상을 강화한다. → **GuideStep에 borderless variant는 도입하지 않으며, LandingHowTo의 `<ol>` 클래스만 조정한다.**
- 헤딩 `text-center text-2xl md:text-3xl font-bold tracking-tight`(Features와 동일), 헤딩→그리드 `gap-6`.

**② 프라이버시 철학 `LandingPrivacy` (organism 신설) — 문단형(3카드 아님):**
- **문단형 채택 근거:** (1) Features·HowTo가 이미 카드 그리드 → 세 번째 그리드는 리듬 단조. (2) 프라이버시는 기능 나열이 아니라 **약속/철학** — 문단형이 warm/personal·신뢰 톤에 맞음. (3) 텍스트-퍼스트 제약과 정합.
- **`bg-brand-soft` 라운드 패널**(radius-2xl, `p-8 md:p-10`, **소프트 면 단일 강조** — 테두리·그림자 없음) 안에: 헤딩 + 리드 2~3문단(`text-base leading-relaxed text-text-secondary`, 문단 간 `gap-3`). planner가 원칙 3개를 구조화해 주면 **카드 아닌 텍스트 행 리스트**(`<ul>`, 각 항목 = 볼드 소제목 + 한 줄)로 — 그리드화 금지.
- 브랜드소프트 채운 패널이 위 열린 그리드들과 대비되어 리듬을 닫는다(닫는 신뢰 모먼트 = 브랜드색 온기).

**③ /guide·/faq·/privacy 연결 마이크로카피(시각 통일):** 기존 `LandingFeatures` 하단 인라인 링크 관례를 **모든 섹션에 통일** — 문장 `text-sm text-text-secondary` + 링크 `text-brand underline underline-offset-2 hover:text-brand-pressed`. 이 2차 링크는 **버튼화 금지**(버튼은 Hero 주 CTA 전용).
- HowTo 하단: "전체 사용법은 사용 가이드에서 →"(→`/guide`). Features의 /guide 링크와 중복되므로 web-dev가 Features 하단 링크를 /faq 위주로 정리(선택).
- Privacy 하단: "자세한 내용은 개인정보처리방침에서 →"(→`/privacy`) — 신뢰 + 크롤 링크.

**컴포넌트:** 신규 organism 2개(`LandingHowTo`·`LandingPrivacy`)만. molecule/atom 신설 없음(GuideStep·Button·Link 재사용, 프라이버시 문단은 시맨틱 마크업 인라인). 카피 const는 `LandingFeatures` 선례대로 organism 내 인라인(planner가 편집) — 또는 `src/content/landing.ts`.
