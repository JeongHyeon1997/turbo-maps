...
- **`GuideStep` molecule 재사용**(신설 최소화) — `<ol className="grid gap-4 sm:grid-cols-2 md:gap-6 lg:grid-cols-3">`로 배치(랜딩은 3~4스텝 축약본, /guide 5스텝과 별개 짧은 카피). GuideStep이 "N. " 접두를 파싱해 번호 뱃지를 만드므로 카피는 동일 포맷.
- 리듬 주의: Features(이모지 4-up 그리드) 바로 뒤 HowTo(번호 3-up 그리드)가 **인접 그리드 2연속** — h2 헤딩 + 뱃지(번호 vs 이모지) + 컬럼 수 차이로 구분. (더 대비가 필요하면 후속에 borderless 변형 검토, 지금은 재사용 우선.)
