...
4. [ ] **app-dev: 모바일 앱(Expo) — Phase 1 뷰 트랙(STEP 0~5) 완료, 남은 건 STEP 6 범위확정(planner)·잔여** — STEP 0~5 전부 Done(아래). **남은 것: ① STEP 6(기록 작성 흐름 — planner 범위확정 필요, Kakao 장소검색 REST 키/프록시 전제) ② 실기기 검증(OAuth 딥링크 Blocked) ③ 프로필 아바타 편집 이식(11 잔여, STEP 5) ④ STEP 5 reviewer Low 1건(수동 refresh/retry 언마운트 취소 미연동 — React18 무해, 09 STEP 6 부근 후속).** → `docs/plan/09-mobile.md`.
...
6. [ ] **웹 성능 최적화 — STEP B~E 전부 완료, 남은 건 STEP A·실측(사용자)** — `docs/plan/12-performance.md`. STEP B(폰트)+C(페칭/캐시)+D(이미지)+E(잔여) 완료(아래 Done). **남은 것: STEP A(리전 확인)·실측(Speed Insights) 둘 다 사용자/대시보드 필요 → Blocked.** STEP F(dba·규모 후)는 현 인덱스로 커버돼 급하지 않음. ※ `notFound()` HTTP 404 status는 Next15 동적 스트리밍 제약으로 보류(Route Handler 전환 외 해법 없음, 계획서 STEP E 참고).
...
## 진행중 (Doing)
- **⏸ 사용자 중단 지점(2026-07-12): mobile STEP 5 + 성능 STEP E 완료 → 코드 트랙이 사실상 사용자 액션 대기 상태.** **09 Phase 1 뷰 트랙(STEP 0~5)·12 성능(STEP B~E) 모두 완료.** 다음 착수 후보: **①0009~0011 라이브 적용(사용자 명시 승인) ②mobile STEP 6 범위 확정(planner 몫 — 기록 작성 흐름·Kakao 장소검색 전제) ③STEP 0 실데이터(사용자) ④리전 확인/실측(사용자·대시보드).** ⇒ 코드로 바로 밀 수 있는 큰 건은 mobile STEP 6 범위확정(planner)뿐이고, 나머지는 전부 사용자/대시보드 액션 대기.
  - **mobile STEP 5 완료(`e998bb3`)** — `logs/[id]` 상세(CoverHero·평점·메모·지도·방문장소·갤러리, 웹 동일 쿼리) + (tabs)/map 실구현(전체 방문 장소 마커) + (tabs)/calendar 실구현(CalendarView, UTC 함정 회피) + **KakaoMap organism(WebView+인라인 HTML, XSS 이스케이프 확인·ready/error postMessage·키없음/실패 graceful 폴백·외부 내비 차단)**. STEP 4 Low 후속 2건(세션 가드 대칭·useFocusEffect+RefreshControl) 4개 화면 전부 처리. reviewer clean — Low3 중 2건(WebView 내비 가드·BackLink 44px·커버 접근성 라벨) 반영, **Low1(수동 refresh/retry 언마운트 취소 미연동 — React18 무해)은 09 STEP 6 부근 후속.** typecheck/lint/expo export PASS. → **09 Phase 1 뷰 트랙 완료.**
  - **성능 STEP E 완료(`4b9760b`)** — 홈 date_logs `limit(20)`+`count exact`(헤드라인 총계 유지) + AppShell 아바타 `HeaderAvatars`+Suspense 분리(SiteHeader authSlot) + 상세 3라우트 generateMetadata `notFound()`(noindex 메타 정확) + axios·zustand 제거 + AdSense lazyOnload. 미들웨어 fast-path는 회귀 위험으로 스킵. **`notFound()` HTTP 404 status는 Next15 동적 스트리밍 프레임워크 제약으로 보류(Route Handler 전환 외 해법 없음).** reviewer PASS. → **12 성능 STEP B~E 완료.**
  - **직전 라운드들(`687c1fb`·`b0a1569`·`bc040e6`·`9eb7420`·`38a9a21`)·프로필/성능 STEP B·mobile STEP 2~4** — 아래 Done. avatar/DB 라이브적용·성능 리전은 사용자/대시보드 대기.
...
- **모바일 앱(Expo) Phase 1 뷰 트랙(STEP 0~5) 완료(2026-07-12)** — `docs/plan/09-mobile.md` STEP 0~5 전부 Done(각 reviewer 지적 반영까지, 아래 Done). **다음: STEP 6(기록 작성) 범위확정=planner 몫, 실기기 검증=OAuth 딥링크 Blocked, 아바타 편집 이식=11 잔여.**
- **다음 세션 시작점:** **코드 트랙이 사실상 사용자 액션 대기.** 코드로 밀 수 있는 건 mobile STEP 6 범위확정(planner). 나머지(STEP 0 실데이터·0009~0011 라이브적용·리전확인/실측·OAuth 딥링크 등록)는 전부 사용자/대시보드 대기.
...
### mobile STEP 5 + 성능 STEP E — 코드 트랙 마무리 (2026-07-12 세션)
- [x] **mobile STEP 5 — 상세+갤러리+지도(WebView) (커밋 `e998bb3`, `docs/plan/09`)** — `logs/[id]` 상세(CoverHero·평점·메모·지도·방문장소·갤러리, 웹 동일 쿼리) + (tabs)/map(전체 방문 장소 마커) + (tabs)/calendar(CalendarView, UTC 함정 회피 확인) + **KakaoMap organism**(WebView+인라인 HTML, XSS 이스케이프 확인·ready/error postMessage·키없음/실패 graceful 폴백·외부 내비 차단). STEP 4 Low 후속 2건(세션 가드 대칭·useFocusEffect+RefreshControl) 4개 화면 전부 처리. reviewer clean(Low3 중 2건 반영: WebView 내비 가드·BackLink 44px·커버 접근성 라벨). **잔여 Low1: 수동 refresh/retry 시그널 언마운트 취소 미연동(React18 무해) → 09 STEP 6 부근 후속.** typecheck/lint/expo export PASS. **→ 09 Phase 1 뷰 트랙(STEP 0~5) 완료.**
- [x] **성능 STEP E — 잔여 (커밋 `4b9760b`, `docs/plan/12`)** — 홈 date_logs `limit(20)`+`count exact`(헤드라인 총계 유지 — 검수 Low 반영) + AppShell 아바타 `HeaderAvatars`+Suspense 분리(SiteHeader authSlot) + 상세 3라우트 generateMetadata `notFound()`(noindex 메타 정확) + axios·zustand 제거 + AdSense lazyOnload. 미들웨어 fast-path는 회귀 위험으로 스킵. **`notFound()` HTTP 404 status는 Next15 동적 스트리밍 프레임워크 제약으로 보류**(Route Handler 전환 외 해법 없음 — Blocked성). reviewer PASS. **→ 12 성능 STEP B~E 완료(남은 건 STEP A·실측 = 사용자/대시보드).**

### "다음작업 진행" 라운드 — 성능 STEP D + mobile STEP 4 (2026-07-12 세션)
- [x] **성능 STEP D — 이미지 (커밋 `687c1fb`, `docs/plan/12`)** — `next.config` `images.remotePatterns`(`NEXT_PUBLIC_SUPABASE_URL`에서 호스트 파생) + `isPublicCoverUrl` 분기로 **public-covers만 next/image**(fill+sizes, 상세 priority) 전환, 서명 date-photos는 raw img 유지(캐시 키 변동 회피) + 갤러리/썸네일 lazy·decoding. **reviewer clean — Low2(CoverHero sizes 상한·호스트 하드코딩) 둘 다 반영.** 다음 STEP E(잔여).
- [x] **mobile STEP 4 — 탭 셸 + 피드 + 프로필 (커밋 `b0a1569`, `docs/plan/09`)** — `(tabs)/_layout.tsx` 4탭(home/map/calendar/profile, map·calendar는 STEP 5 자리표시자) + `home.tsx`(웹 동일 date-log 쿼리 + RLS 커플스코프 + 서명 커버 + FlatList) + `profile.tsx`(닉네임·커플상태·ThemeToggle·로그아웃) + atoms 4·molecules 2 신규 + 부팅게이트 connected→`/home` 실연결. typecheck/lint/expo export PASS. **reviewer clean — Low2는 09 STEP 5 범위 후속 기록**(①홈 탭 세션만료 리다이렉트 비대칭 ②탭 포커스 재조회/RefreshControl 없음).
...
- [x] **AvatarUploader + 배선 (커밋 `b2dbd08`)** — `AvatarUploader` molecule(미리보기·업로드·"카카오 사진으로 되돌리기") + `/profile`·`AppShell` 렌더 코얼레스(`custom_avatar_url ?? avatar_url`) + `select('*')` degrade(컬럼 미적용 환경 방어) + shared `customAvatarUrl` Zod. `auth/callback`은 설계대로 무변경(custom 미접촉=폴백 유지). **reviewer PASS — Med2·Low1 전건 반영**(연속 업로드 고아 객체 정리, 버킷 제약, update 실패 시 고아 방지). typecheck/lint/build PASS.

### 웹 성능 12 — 계획서 + STEP B (2026-07-12 세션, `docs/plan/12`)
- [x] **성능 감사 계획서화 (커밋 `fa61d79`)** — 감사 원문(우선순위표 15항목·상세·권장순서) 보존 + STEP A~F 분해. 11번 아바타 배선과 AppShell/server.ts/이미지 충돌 → STEP C·D는 11 web STEP 완료 후 명시.
- [x] **STEP B — 폰트 (커밋 `516584a`)** — globals.css `@import` 제거 → layout preconnect + 버전고정 링크(pretendard@1.3.9). **버그 수정: fontFamily `'Pretendard'` ↔ CDN 선언 `'Pretendard Variable'` 불일치로 본문이 시스템 sans로 폴백 중이던 것 교정**(sans/jua/logo 정합). BMJUA **1.5MB→18KB** pyftsubset 서브셋 + 미사용 TTF 4종 삭제(감사 15). 빌드+next start 실렌더 검증 완료. **다음 착수점 = STEP C(페칭/캐시).**
