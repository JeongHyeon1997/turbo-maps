...
> **진행: Phase 1 뷰 트랙 STEP 0~5 전부 완료(최신 `e998bb3`).** 다음 = STEP 6(기록 작성, planner 범위확정 필요). 남은 것: STEP 6 + 실기기 검증(OAuth 딥링크 Blocked) + 프로필 아바타 편집 이식(11 잔여).
>
...
### STEP 5 — 상세 + 사진 갤러리 + 지도(WebView) (app-dev) [완료 `e998bb3`]
- `app/logs/[id].tsx` — 상세(CoverHero·평점·메모·지도·방문장소 리스트·갤러리, 웹 동일 쿼리). (tabs)/map 실구현(전체 방문 장소 마커), (tabs)/calendar 실구현(CalendarView, **UTC 함정 회피 확인**).
- **KakaoMap organism(RN/WebView)**: WebView + 인라인 HTML에 Kakao JS SDK 로드 → 마커 + 경로 Polyline. **XSS 이스케이프 확인 · ready/error postMessage · 키없음/실패 graceful 폴백 · 외부 내비 차단.**
- **STEP 4 reviewer Low 2건 처리 완료(4개 화면 전부):** ① 홈 탭 세션만료 로그인 리다이렉트 가드 대칭. ② `useFocusEffect` 재조회 + `RefreshControl` pull-to-refresh.
- **reviewer clean:** Low3 중 2건(WebView 내비 가드·BackLink 44px·커버 접근성 라벨) 반영. **잔여 Low1(수동 refresh/retry 시그널 언마운트 취소 미연동 — React18 무해) → STEP 6에서 함께 처리.**
- **완료 기준:** 상세 지도(마커/경로)·갤러리·평점·메모 표시, 지도 터치 팬/줌 동작. typecheck/lint/expo export PASS.

### STEP 6 — 작성(logs/new, 다중 사진) (app-dev) [후속, planner 범위확정 필요]
- **⚠️ planner 범위확정 선행:** Kakao 장소검색 REST 키/프록시 전제(현재 미충족 — PROGRESS `## 다음` 8번). 전제 미충족 시 STEP 5.5로 좌표 수동/생략 대안 검토.
...
- **STEP 5 잔여 Low1 여기서 처리:** 수동 refresh/retry 시그널을 언마운트 취소(AbortController/isMounted)에 연동.
- 프로필 아바타 편집 이식(11 잔여, `expo-image-picker`+`avatars` 버킷)도 이 STEP 부근에서 함께 검토.
