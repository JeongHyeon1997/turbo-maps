...
### STEP 3 — 커플 연결 (app-dev)
- `app/(auth)/couple-connect.tsx` — 웹 미러: 내 초대코드 생성/표시(복사) + 상대 코드 입력→`join_couple`. 성공 시 (tabs)/home.
- 커플 상태에 따른 부팅 게이트(STEP 2와 연결).
- **완료 기준:** 두 계정 실테스트로 연결(웹 Blocked 항목과 겸함), 연결 후 커플 스코프 데이터 조회.

### STEP 4 — 탭 셸 + 피드 + 프로필 (app-dev)
- `app/(tabs)/_layout.tsx` — 하단 탭(home/map/calendar/profile), 웹 BottomNav IA 미러. `@react-navigation/bottom-tabs`(설치됨) 또는 expo-router Tabs.
- `home.tsx` — 커플 date-log 조회 → `DateLogCard`(molecule, RN판). 사진 커버·제목·날짜·장소수·평점. 빈 상태.
- `profile.tsx` — 닉네임·커플상태·**테마 토글(STEP 1)**·로그아웃.
- **완료 기준:** 웹에서 만든 기록이 폰 피드에 뜸, 탭 이동, 로그아웃 동작.
